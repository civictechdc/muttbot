    ...
    robot.respond /(metro|wmata)(\s+(.+))?/i, (msg) ->
        ...
        if msg.match[3]
            ...
            if search.match /^(\d\d|\w\d)$/i
                msg.http("http://api.wmata.com/Incidents.svc/json/BusIncidents?Route=#{search}&api_key=xqv42tjfvz4abb9y9averdvd")
                .get() (err, resp, body) ->
                    arr = []
            ...
            # Bus arrivals by 7-digit stop ID
                ...
                msg.http("http://api.wmata.com/NextBusService.svc/json/jPredictions?StopID=#{search}&api_key=xqv42tjfvz4abb9y9averdvd")
                .get() (err, resp, body) ->
                    arr = []
                    results = JSON.parse body
            ...
            # Nearby bus and train stations
                ...
                results = geocoder.geocode "#{search} washington, dc", (err, resp) ->
                    ...
                    msg.http("http://api.wmata.com/rail.svc/json/jStationEntrances?lat=#{resp[0].latitude}&lon=#{resp[0].longitude}&radius=200&api_key=xqv42tjfvz4abb9y9averdvd")
                    .get() (err, res, body) ->
                        results = JSON.parse body
                        ...
                        for i in results.Entrances
                            if i.StationCode1 not in s
                                msg.http("http://api.wmata.com/Rail.svc/json/jStationInfo?StationCode=#{i.StationCode1}&api_key=xqv42tjfvz4abb9y9averdvd")
                                .get() (err, resp, body) ->
                                    results = JSON.parse body
                                    arr += "#{results.Name}\n"
                                s.push i.StationCode1
                        msg.http("http://api.wmata.com/Bus.svc/json/jStops?lat=#{resp[0].latitude}&lon=#{resp[0].longitude}&radius=200&api_key=xqv42tjfvz4abb9y9averdvd")
                        .get() (err, res, body) ->
                            results = JSON.parse body
            ...
            # Train incident by line color
                ...
                lines =
                    ...
                    "yellow": "YL;"
                    "silver": "SV;"
                msg.http("http://api.wmata.com/Incidents.svc/json/Incidents?api_key=xqv42tjfvz4abb9y9averdvd")
                .get() (err, resp, body) ->
                    results = JSON.parse body
            ...
            # Train arrivals by station code (one letter and two numbers)
                ...
                arr = ""
                msg.http("http://api.wmata.com/Rail.svc/json/jStationInfo?StationCode=#{stations[station[0]][0]}&api_key=xqv42tjfvz4abb9y9averdvd")
                .get() (err, resp, body) ->
                    results = JSON.parse body
                    arr += "Here are the predictions for #{results.Name}:\n\n"
                    for s in stations[station[0]]
                        msg.http("http://api.wmata.com/StationPrediction.svc/json/GetPrediction/#{s}?api_key=xqv42tjfvz4abb9y9averdvd")
                        .get() (err, resp, body) ->
                            results = JSON.parse body
